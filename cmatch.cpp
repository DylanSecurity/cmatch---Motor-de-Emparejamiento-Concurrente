        ...
        while (!match_found && tournament.is_running) {
            ...
            if (best_opponent_id != -1) {
                ...
                if (available_board_id != -1) {
                    ...
                    while (!assigned_board.game_finished) {
                ...
                if (me.state == MATCHED && me.assigned_board != -1) {
                match_found = true;
                Board& assigned_board = tournament.boards[me.assigned_board];
            
                // Yo (el oponente) también espero a que termine la partida
                while (!assigned_board.game_finished) {
                pthread_cond_wait(&assigned_board.board_sync, &tournament.sync_mutex);
                 }
            
                // Yo (el oponente) tiro mis propios dados para ver si reingreso
                float rand_prob = (float)rand() / RAND_MAX;
                if (rand_prob <= app_config.reenter_probability) {
                me.state = WAITING;
                me.opponent_id = -1;
                ...
                continue; // Vuelvo al inicio para salir del bucle de búsqueda
                }
